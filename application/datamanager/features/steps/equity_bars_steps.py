...
def step_impl(context):
    # Set environment variables for testing if they don't exist
    if not os.environ.get("POLYGON_API_KEY"):
        os.environ["POLYGON_API_KEY"] = "dummy-test-key"

    if not os.environ.get("POLYGON_BASE_URL"):
        os.environ["POLYGON_BASE_URL"] = "http://localhost:8000"
...
def step_impl(context, date_str):
    ...
    if os.environ.get("GCP_GCS_BUCKET"):
        ...
        expected_file = Path(f"equity_bars_{date_str}.parquet")
        assert not expected_file.exists(), f"Parquet file {expected_file} still exists after deletion"

@then("the response should confirm successful deletion")
def step_impl(context):
    response_json = context.response.json()
    assert "status" in response_json, "Response missing status field"
    assert response_json["status"] == "success", f"Expected success status, got {response_json['status']}"
    assert "date" in response_json, "Response missing date field"
    assert "message" in response_json, "Response missing message field"
    assert "deleted successfully" in response_json["message"], "Response message does not confirm deletion"