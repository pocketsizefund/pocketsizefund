...
@when(
    'I send a GET request to "{endpoint}" for date range "{start_date}" to "{end_date}")'
)
def step_imp(context, endpoint, start_date, end_date):
    url = f"{context.api_url}{endpoint}"
    start_date = datetime.strptime(start_date, "%Y/%m/%d").date()
    end_date = datetime.strptime(end_date, "%Y/%m/%d").date()
    response = requests.get(
        url, params={"date_range": {"start": start_date, "end": end_date}}
    )
    context.response = response
