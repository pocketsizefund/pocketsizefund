...
def delete_bars(date: date, settings: Settings) -> bool:
    path = (
        f"gs://{settings.gcp_gcs_bucket}/equity/bars/{date.strftime('%Y-%m-%d')}/data.parquet"
        if settings.gcp_gcs_bucket
        else f"equity_bars_{date.strftime('%Y-%m-%d')}.parquet"
    ...
    # For local files, use pathlib
    if not settings.gcp_gcs_bucket:
        file_path = Path(path)
        if file_path.exists():
            file_path.unlink()
            return True
    ...
    # For GCS bucket, use DuckDB httpfs
    connection = get_connection(settings)
    # Check if file exists first by trying to read metadata
    try:
        connection.execute(f"SELECT COUNT(*) FROM '{path}'")
        # If we reach here, file exists, so delete it
        connection.execute(f"DROP TABLE IF EXISTS '{path}'")
        return True
    except Exception:
        # File doesn't exist or another error occurred
        return False
...
async def get_equity_bars(
    ...
    results = query_bars(date_range.start, date_range.end, settings)
    return {
...
async def delete_equity_bars(request: Request, date: date) -> dict[str, object]:
    settings: Settings = request.app.state.settings
    ...
    return {
        "status": "success",
        "date": str(date),
        "message": f"Equity bars data for {date} deleted successfully"
    }
