...
class Instance(BaseModel):
    ...
    bundle_manager: str = "medium_2_0"
    bundle_worker: str = "small_2_0"
    public_ip: str | None = None
    tailscale_ip: str | None = None


def remove_instance(instance: Instance) -> None:
    ls = boto3.client("lightsail", region_name=instance.region)

    try:
        ls.delete_instance(instanceName=instance.name)
...
def release_static_ip(instance: Instance) -> None:
    ...
    try:
        ls.release_static_ip(staticIpName=ip_name)
    except ClientError as e:
        if e.response["Error"]["Code"] != "NotFoundException":
...
def ensure_ls_keypair() -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    try:
        ls.get_key_pair(keyPairName=LIGHTSAIL_KEY_NAME)
        logger.info(
        ...
        ls.delete_key_pair(keyPairName=LIGHTSAIL_KEY_NAME)
    ...
    try:
        ...
        resp = ls.create_key_pair(keyPairName=LIGHTSAIL_KEY_NAME)
...
def create_instance(instance: Instance) -> Instance:
    ...
    try:
        existing = ls.get_instance(instanceName=instance.name)
        ...
        while state != "running":
            ...
            time.sleep(2)
            result = ls.get_instance(instanceName=instance.name)
    ...
    try:
        ls.create_instances(
            ...
            bundleId=instance.bundle_manager,
    ...
    state = "pending"
    while state != "running":
        state = ls.get_instance(instanceName=instance.name)["instance"]["state"]["name"]
        time.sleep(0.25)
...
def create_network(instance: Instance) -> Instance:
    ...
    try:
        ls.attach_static_ip(staticIpName=ip_name, instanceName=instance.name)
    ...
    instance.public_ip = ls.get_static_ip(staticIpName=ip_name)["staticIp"]["ipAddress"]
    return instance
...
def run_setup_script(instance: Instance) -> Instance:
    ...
    with Connection(
        ...
        ssh.run("sudo apt-get update && sudo apt-get -y upgrade")
...
def create_docker_context(manager: Instance) -> None:
    ...
    subprocess.run(
        [docker, "context", "rm", "pocketsizefund", "--force"],  # noqa: S603
        check=False,
        capture_output=True,
    )
    ...
    if result.returncode == 0:
        ...
        subprocess.run(
            [docker, "context", "use", "pocketsizefund"],  # noqa: S603
            capture_output=True,
            check=False,
        )
...
def down() -> None:
    ...
    manager = Instance(name="swarm-manager-0")
...
def up() -> None:
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        results = []
        for instance, future in zip(instances, futures, strict=False):
            try:
