...
from internal.cloud_event import (
    create_cloud_event_error,
    create_cloud_event_success,
)
from internal.dates import DateRange
...
def open_position(event: CloudEvent) -> CloudEvent:
    ...
    try:
        ...
        return create_cloud_event_error(
            ...
            error_message=f"Error opening position: {e!r}",
        )

    eastern_timezone = ZoneInfo("America/New_York")

    end_datetime = datetime.now(tz=eastern_timezone)
    start_datetime = end_datetime - timedelta(days=trading_days_per_year)
    ...
    try:
        ...
        return create_cloud_event_error(
            application_name="positionmanager",
            event_metadata=["positions", "open"],
            error_message=f"Error fetching historical data: {e!r}",
        )
    ...
    try:
        ...
        return create_cloud_event_error(
            application_name="positionmanager",
            event_metadata=["positions", "open"],
            error_message=f"Error optimizing portfolio: {e!r}",
        )
...
def close_positions() -> CloudEvent:
    ...
    try:
        ...
        return create_cloud_event_error(
            application_name="positionmanager",
            event_metadata=["positions", "close"],
            error_message=str(e),
        )
    ...
    return create_cloud_event_success(
        application_name="positionmanager",
        event_metadata=["positions", "closed"],
        data={
