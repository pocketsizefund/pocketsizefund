...
from internal.summaries import BarsSummary
from pydantic import ValidationError


class TestDate(unittest.TestCase):
    def test_date_initialization_default(self) -> None:
        date_with_default = Date()
        assert isinstance(date_with_default.date, date)

    def test_date_initialization_with_date(self) -> None:
        test_date = date(2023, 5, 15)
        date_with_value = Date(date=test_date)
        assert date_with_value.date == test_date

    def test_date_string_parsing_iso_format(self) -> None:
        date_with_string = Date(date="2023-5-15")  # type: ignore
        assert date_with_string.date == date(2023, 5, 15)

    def test_date_string_parsing_slash_format(self) -> None:
        date_with_slash = Date(date="2023/05/15")  # type: ignore
        assert date_with_slash.date == date(2023, 5, 15)

    def test_date_invalid_format(self) -> None:
        with pytest.raises(ValidationError, match="Invalid date format"):
            Date(date="invalid-date")  # type: ignore

    def test_date_invalid_date_values(self) -> None:
        with pytest.raises(ValidationError):
            Date(date="2023-13-01")  # type: ignore

    def test_date_json_encoder(self) -> None:
        test_date = date(2023, 5, 15)
        date_instance = Date(date=test_date)
        json_data = date_instance.model_dump(mode="json")
...
class TestModelIntegration(unittest.TestCase):
    def test_date_to_bars_summary(self) -> None:
        date_instance = Date(date="2023-05-15")  # type: ignore
        bars_summary = BarsSummary(
            date=date_instance.date.strftime("%Y-%m-%d"), count=100
        )
    ...
    def test_multiple_model_validation(self) -> None:
        ...
        assert date_instance.date == date(2023, 5, 15)
