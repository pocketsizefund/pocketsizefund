...
async fn query_s3_parquet_data(
    ...
    let portfolio = statement.query_row([], |row| {
        Ok(Portfolio {
            ticker: row.get(0)?,
            timestamp: row.get(1)?,
            side: row.get(2)?,
    ...
    df!(
        "ticker" => vec![portfolio.ticker],
        "timestamp" => vec![portfolio.timestamp],
        "side" => vec![portfolio.side],
        "dollar_amount" => vec![portfolio.dollar_amount],
    )
