...
def fetch_equity_bars(
    ...
    if fetch_date is None:
        fetch_date = Date(date=datetime.now(tz=ZoneInfo("America/New_York")).date())

    request_fetch_date: str = fetch_date.date.strftime("%Y-%m-%d")
    ...
    if count > 0:
        try:
            ...
            return create_cloud_event_error(
                ...
                event_metadata=["equity", "bars", "errored"],
                error_message=f"Error writing parquet file: {e!s}",
            )
...
def delete_equity_bars(request: Request, fetch_date: Date) -> Response:
    s3_client = request.app.state.s3_client
    year = fetch_date.date.year
    month = fetch_date.date.month
    day = fetch_date.date.day
