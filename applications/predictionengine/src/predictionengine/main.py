...
from internal.miniature_temporal_fusion_transformer import (
    ...
    try:
        ...
        if data.is_empty():
            return create_cloud_event_error(
                application_name="predictionengine",
                event_metadata=["predictions", "skipped"],
                error_message="No data available for prediction",
            )
        ...
        if not predictions:
            return create_cloud_event_error(
                application_name="predictionengine",
                event_metadata=["predictions", "errored"],
                error_message="No predictions could be generated",
            )

        return create_cloud_event_success(
            application_name="predictionengine",
            event_metadata=["predictions", "created"],
            data={
