...
from pricemodel.pipeline.model import TemporalFusionTransformer
...
def prepare_data(data_path) -> pl.DataFrame:
    ...
    data = data.with_columns([
        ...
        (pl.col("timestamp").dt.weekday() >= 5).alias("is_weekend").cast(pl.Int32),
    ...
    data = data.with_columns([
        pl.Series(name="ticker", values=ticker_encoded)
    ])
    
    console.print("Computing static real variables per ticker...", style="cyan")
    static_real_features = data.group_by("ticker").agg([
    ...
    data = data.join(static_real_features, on="ticker", how="left")
    ...
    # training_data = data.filter(pl.col("timestamp") < training_cutoff).drop(["timestamp"])
    # validation_data = data.filter(pl.col("timestamp") >= training_cutoff).drop(["timestamp"])
    #
    # training_dates = set(training_data.select("time_idx").unique().collect()["time_idx"].to_list())
    # validation_dates = set(validation_data.select("time_idx").unique().collect()["time_idx"].to_list())
    ...
    encoder_length = 30
    decoder_length = 5
    max_time_idx = data.select(pl.col("time_idx")).max().collect()["time_idx"][0]
    features=["ticker", "relative_time_idx", "time_idx",
                                    "open", "high", "low", "close",
    ...
    progress_group = Group(step_progress)

    i = 0

    training_sequences_x = []
    training_sequences_y = []
    validation_sequences_x = []
    validation_sequences_y = []
    with Live(progress_group, refresh_per_second=20):
        step_task = step_progress.add_task("steps", total=data.collect().shape[0])
        for ticker, group in data.collect().group_by("ticker"):
            group = group.sort("time_idx")
            total_length = len(group)

            for i in range(total_length - encoder_length - decoder_length + 1):
                encoder_data = group[i: i + encoder_length].select(pl.col(feat) for feat in features)
                decoder_data =group[i + encoder_length: i + encoder_length + decoder_length].select(pl.col(tgt) for tgt in target)
                if group["time_idx"].max() < max_time_idx - 40:
                    training_sequences_x.append(encoder_data.to_numpy())
                    training_sequences_y.append(decoder_data.to_numpy())
                else:
                    validation_sequences_x.append(encoder_data.to_numpy())
                    validation_sequences_y.append(decoder_data.to_numpy())


                step_progress.update(step_task, advance=1)
    ...
    training_data = Tensor(training_sequences_x), Tensor(training_sequences_y)
    validation_data  = Tensor(validation_sequences_x), Tensor(validation_sequences_y)

    return SimpleNamespace(sets=SimpleNamespace(training=training_data,
...
class TimeSeriesDataset(mlflow.pyfunc.PythonModel):
    def __init__(self, data, features: list[str], target: list[str]):
        self.artifacts = data.artifacts
        self.features = features
        self.target = target

        self.train_x = Tensor(data.sets.training.select([pl.col(feature) for feature in self.features]).collect().to_numpy())
        self.train_y = Tensor(data.sets.training.select([pl.col(target) for target in self.target]).collect().to_numpy())
        self.validation_x = Tensor(data.sets.validation.select([pl.col(feature) for feature in self.features]).collect().to_numpy())
        self.validation_y = Tensor(data.sets.validation.select([pl.col(target) for target in self.target]).collect().to_numpy())
    ...
    def prepare_inference_data(self, tickers, start_date, end_date):
        ...
        inference_df = inference_df.join(static_features_pl, on="ticker", how="left")
...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        data = prepare_data("data/previous-close/sample.csv")
        # data.save_checkpoint(mlflow, run)

        trainer = Trainer(
            experiment=experiment,
            model=TemporalFusionTransformer,
            hyperparameters={"input_dim": 12, "num_tickers": 30, "embedding_dim": 100},
            optimizer=nn.optim.Adam,
            train=data.sets.training,
            validation=data.sets.validation,
            max_epochs=10,
            batch_size=128
        )
