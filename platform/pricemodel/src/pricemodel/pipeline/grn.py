...
class TimeDistributedInterpolation:
    ...
    def __call__(self, x):
        ...
        x_ = (
                ...
                .view(-1, x.size(-1))
                .unsqueeze(1)
                .interpolate((self.output_size, ), mode="linear", align_corners=True).squeeze(1)
                .squeeze(1)
            )
...
class GatedAddNorm:
    def __init__(
        ...
        self.add_norm = AddNorm(self.output_size, skip_size=self.skip_size, trainable_add=trainable_add)
...
class ResampleNorm:
    def __init__(self, input_size: int,
                 output_size: int | None = None, trainable_add: bool = True):
        self.input_size = input_size
        self.trainable_add = trainable_add
        self.output_size = output_size or input_size
