...
class Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        self.train_steps = train.num_steps
    ...
    def val_loss(self) -> Tensor:
        x_test, y_test = self.validation.batch()
    ...
    def __call__(self):
        ...
        experiment = mlflow_client.search_experiments(
            filter_string=f"name = '{self.experiment.name}'"
        )
        if not experiment:
            mlflow_client.create_experiment(name=self.experiment.name, tags=self.experiment.tags)
