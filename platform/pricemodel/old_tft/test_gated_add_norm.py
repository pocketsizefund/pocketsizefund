...
def tensors(draw):
    return Tensor(
        draw(
            arrays(dtype=floating_dtypes(), shape=array_shapes(min_dims=2, max_dims=2, min_side=1))
        )
    )
...
def test_gated_add_norm_different_dropout(input_tensor):
    ...
    assert (
        abs(result1 - result2).mean().item() > 1e-6
    )  # Expect different results due to different dropout
