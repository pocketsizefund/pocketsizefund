...
def prepare_data(data: pl.LazyFrame) -> pl.DataFrame:
    ...
    valid_tickers = ticker_counts.filter(pl.col("count") >= MIN_OBSERVATIONS_PER_TICKER).select(
        "ticker"
    )
    ...
    latest_timestamps = data.group_by("ticker").agg(
        pl.col("timestamp").max().alias("max_timestamp")
    )
    latest_timestamps = latest_timestamps.with_columns([pl.col("max_timestamp")])
    valid_tickers = latest_timestamps.filter(pl.col("max_timestamp") >= freshness_dt)["ticker"]
    data = data.filter(pl.col("ticker").is_in(valid_tickers))
