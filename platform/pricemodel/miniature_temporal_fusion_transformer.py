...
class MiniatureTemporalFusionTransformer:
    ...
    def forward(
        ...
        lstm_input = features.concat(ticker_embeddings, axis=-1)

        lstm_output = self.lstm_encoder.forward(lstm_input)
        ...
        percentile_25, percentile_50, percentile_75 = self.post_processor.post_process_predictions(
            tickers.numpy(),
            output.numpy(),
        )
    ...
    def predict(
        ...
        predictions = self.forward(tickers, input)
