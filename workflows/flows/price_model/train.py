...
from workflows.schema.polygon import Bars
import polars as pl
from datetime import date


dataframes = ImageSpec(
    registry="docker.io",
    name="pocketsizefund/dataframes",
...
def read_bars() -> Bars:  # TODO: make this not optional, figure out better conditional flow
    ...
    # TODO: probably should put this in a /bars/ prefix
    path = f"s3://{BUCKET}/data/*/*/*/*.parquet"

    # TODO: set default region to remove polars warning
...
def process_bars(bars: Bars) -> Bars:
    return bars.sort(["ticker", "date"]).unique(subset=["ticker", "date"])


@task(container_image=dataframes)
def filter_tickers_by_minimum_observations(
    bars: Bars, minimum_observations_per_ticker: int
) -> Bars:
    ticker_counts = bars.group_by("ticker").agg(pl.count().alias("count"))
...
@task(container_image=dataframes)
def filter_out_stale_tickers(bars: Bars, freshness_date: date) -> Bars:
    latest_dates = bars.group_by("ticker").agg(pl.col("date").max().alias("max_timestamp"))
    latest_dates = latest_dates.with_columns([pl.col("max_timestamp")])
    valid_tickers = latest_dates.filter(pl.col("max_timestamp") >= freshness_date)["ticker"]
    return bars.filter(pl.col("ticker").is_in(valid_tickers))
...
def main(freshness_date: date = "2013-12-31", minimum_observations_per_ticker: int = 100) -> Bars:
    ...
    bars = filter_tickers_by_minimum_observations(bars, minimum_observations_per_ticker)
...
#         [
#             pl.col("timestamp").rank(method="dense").cast(pl.Int32).alias("time_idx")
#             - 1  # Start from 0
#         ]
#     )
#
...
#     data = data.with_columns([pl.Series(name="ticker_encoded", values=ticker_encoded)])
#
...
#         [
#             pl.when(pl.col(col) == 0).then(1e-8).otherwise(pl.col(col)).alias(col)
#             for col in ["open_scale", "high_scale", "low_scale", "close_scale"]
#         ]
#     )
#
...
#     data = data.with_columns(
#         [
#             ((pl.col("open") - pl.col("open_center")) / pl.col("open_scale")).alias(
#                 "open_normalized"
#             ),
#             ((pl.col("high") - pl.col("high_center")) / pl.col("high_scale")).alias(
#                 "high_normalized"
#             ),
#             ((pl.col("low") - pl.col("low_center")) / pl.col("low_scale")).alias("low_normalized"),
#             ((pl.col("close") - pl.col("close_center")) / pl.col("close_scale")).alias(
#                 "close_normalized"
#             ),
#         ]
#     )
...
#
#     data = data.with_columns([pl.col("timestamp").cast(pl.Date)])
#
...
#
#     assert (
#         len(training_dates.intersection(validation_dates)) == 0
#     ), "Training and validation sets have overlapping dates."
...
#             "close_normalized",
#             "day_of_week_scaled",
#             "week_of_month_scaled",
#             "month_of_year_scaled",
#             "week_of_year_scaled",
#             "is_weekend",
#         ]
